<!DOCTYPE html>
<html lang="fr">

<head>
    <meta charset="UTF-8">
    <link rel="stylesheet" href="styles/general.css">
    <link rel="stylesheet" href="styles/menu.css">
    <link rel="stylesheet" href="styles/contact.css">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <link rel="icon" type="image/png" sizes="192x192" href="img/logo_messerschmitt192.png">
    <link rel="icon" type="image/png" sizes="192x192" href="img/logo_messerschmitt16.png">
    <script src="script/contact.js"></script>
    <title>A propos</title>
</head>

<body>
    <header>
        <div class="conteneur">
            <div class="header-logo">
                <div class="logo">
                    <img src="img/logo.png" alt="logo">
                </div>
            </div>
        </div>
    </header>
    <main>
        <div class="conteneur">
            <p>Votre message &agrave; bien &eacute;t&eacute; envoy&eacute;.</p>
            <p>Je ferais de mon possible pour vous r&eacute;pondre le plus rapidement.</p>
            <p>Merci de votre visite.</p>
            <p>Guillaume Cr&eacute;gut</p>
            <p><a href="index.html">Retour &agrave; l'index</a></p>
        </div>
    </main>
    <footer>
        <div class="conteneur">
            <div>Copyright 2021, G. Crégut, Editiel98</div>
            <nav class="footernav">
                <ul class="menu_footer">
                    <li><a href="apropos.html">Me contacter</a></li>
                </ul>
            </nav>
        </div>
    </footer>
</body>

</html>